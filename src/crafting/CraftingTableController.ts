...
import type { CraftingTableUi } from './CraftingTableUi';
...
import { CraftingTransferService } from './CraftingTransferService';

export class CraftingTableController extends BaseContainerController {
  public readonly craftingGrid = new CraftingGrid(3, 3);
  public resultSlotStack: ItemStack | null = null;

  public constructor(
    inventory: Inventory,
    private readonly ui: CraftingTableUi,
  ...
  private setupListeners(): void {
    ...
    this.ui.getCraftingSlots().forEach((slotEl, idx) => {
      slotEl.addEventListener('mouseenter', (e: MouseEvent) => {
        ...
        if (this.isRightDragging && this.cursorStack !== null) {
          if (!this.dragSlots.has(-200 - idx)) {
            this.dragSlots.add(-200 - idx);
      ...
      slotEl.addEventListener('mouseleave', () => {
        if (!this.isOpen) return;
        if (this.hoveredSlotIndex === -200 - idx) {
          this.hoveredSlotIndex = -1;
          this.ui.hideHoverHighlight();
          this.tooltip.update(0, 0, null, this.scale);
        }
