...
/**
 * Modal crafting table UI window (176x166 texture `crafting_tablemenu.png`).
 * Manages all 36 player slots, 9 3x3 crafting input slots, and 1 crafting result slot.
 */
export class CraftingTableUi {
  ...
  public constructor() {
    ...
    this.root.id = 'crafting-table-modal-root';
    ...
    this.windowEl.className = 'crafting-table-window';
    ...
    this.windowEl.style.background = "url('/textures/gui/crafting_tablemenu.png') center / 100% 100% no-repeat";
    ...
    // Player slots 0..35
    for (let i = 0; i < 36; i++) {
      const e = this.createSlotElement(`table-player-${i}`, String(i));
      this.windowEl.appendChild(e);
      this.slots.push(e);
    }

    // 3x3 Crafting input slots 0..8
    for (let i = 0; i < 9; i++) {
      const e = this.createSlotElement(`table-crafting-${i}`, String(i));
      this.windowEl.appendChild(e);
      this.craftingSlots.push(e);
    }

    // Result slot
    this.resultSlotEl = this.createSlotElement('table-crafting-result', 'result');
  ...
  public setScale(scale: number): void {
    ...
    // Crafting input slots 0..8 (3x3 grid)
    ...
    if (this.resultSlotEl && this.resultSlotEl.style) {
      this.resultSlotEl.style.left = `${124 * scale}px`;
      this.resultSlotEl.style.top = `${35 * scale}px`;
