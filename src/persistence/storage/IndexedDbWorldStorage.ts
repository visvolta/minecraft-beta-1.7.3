...
export class IndexedDbWorldStorage implements WorldStorage {
  ...
  public async delete(worldId: string, key: string): Promise<void> { await this.request('readwrite', store=>store.delete(`${worldId}/${key}`)); }
  public async deleteWorld(worldId: string): Promise<void> { const prefix = `${worldId}/`; await new Promise<void>((resolve,reject)=>{ const tx=this.db.transaction(STORE,'readwrite'); const store=tx.objectStore(STORE); const request=store.openCursor(); request.onsuccess=()=>{const cursor=request.result;if(!cursor)return;if(typeof cursor.key==='string'&&cursor.key.startsWith(prefix))cursor.delete();cursor.continue();}; request.onerror=()=>reject(request.error ?? new Error('IndexedDB cursor failed')); tx.oncomplete=()=>resolve(); tx.onerror=()=>reject(tx.error ?? new Error('IndexedDB delete world failed')); }); }
