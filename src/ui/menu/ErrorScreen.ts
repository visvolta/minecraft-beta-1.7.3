...
export class ErrorScreen extends Screen {
  ...
  public constructor(titleText: string, messageText: string, back: () => void) {
    ...
    const title=document.createElement('div'); title.textContent=titleText; title.style.cssText='position:absolute;left:0;right:0;top:70px;text-align:center;font:18px Minecraft;color:white';
    const msg=document.createElement('div'); msg.textContent=messageText; msg.style.cssText='position:absolute;left:10%;right:10%;top:110px;text-align:center;color:#ff8080;font:12px Minecraft';
