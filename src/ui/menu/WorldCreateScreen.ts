...
export interface WorldCreateResult { readonly name: string; readonly seedText: string; readonly gameMode: GameMode; readonly cheatsEnabled: boolean; }

export class WorldCreateScreen extends Screen {
  private mode = GameMode.Survival;
  /** World-level, never a global setting. */
  ...
  private readonly cheatsButton: GuiButton;
  ...
  public constructor(create: (result: WorldCreateResult) => void, back: () => void) {
    ...
    const title=document.createElement('div'); title.textContent='Create New World'; title.style.cssText='position:absolute;left:0;right:0;top:20px;text-align:center;font:18px Minecraft;color:white';
    ...
    this.cheatsButton=new GuiButton('Allow Cheats: OFF',()=>{this.cheats=!this.cheats;this.cheatsButton.element.textContent=`Allow Cheats: ${this.cheats?'ON':'OFF'}`;},200,20);
    this.createButton=new GuiButton('Create New World',()=>{const trimmed=this.name.value.trim();if(trimmed.length>0)create({name:trimmed,seedText:this.seed.value,gameMode:this.mode,cheatsEnabled:this.cheats});},200,20);
    this.cancel=new GuiButton('Cancel',back,200,20);
    this.root.append(title,this.nameLabel,this.name.element,this.seedLabel,this.seed.element,this.modeButton.element,this.cheatsButton.element,this.createButton.element,this.cancel.element);
  ...
  private layout(): void {
    ...
    this.modeButton.setPosition(x,170); this.cheatsButton.setPosition(x,194); this.createButton.setPosition(w/2-205,h-50); this.cancel.setPosition(w/2+5,h-50);
  }
}
function label(text:string):HTMLDivElement{const e=document.createElement('div');e.textContent=text;e.style.cssText='position:absolute;color:#aaa;font:12px Minecraft';return e;}
