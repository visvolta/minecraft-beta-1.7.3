...
export class ConfirmDeleteScreen extends Screen {
  ...
  public constructor(worldName: string, confirm: () => void, cancel: () => void) {
    ...
    const title=document.createElement('div'); title.textContent='Delete World'; title.style.cssText='position:absolute;left:0;right:0;top:70px;text-align:center;font:18px Minecraft;color:white';
    const msg=document.createElement('div'); msg.textContent=`Are you sure you want to delete "${worldName}"?`; msg.style.cssText='position:absolute;left:10px;right:10px;top:110px;text-align:center;font:12px Minecraft;color:#ddd';
