...
export class DebugOverlay {
  ...
  private formatLines(stats: DebugStats): string[] {
    return [
      ...
      `Gen/mesh time: ${stats.generationTimeMs.toFixed(2)} / ${stats.meshingTimeMs.toFixed(2)} ms`,
      `Fog: ${stats.fogMode}`,
      ...
      `Lighting: prop ${stats.lightingPropagationMs.toFixed(2)} ms  calls ${stats.lightingPropagationCalls}  bfs avg ${stats.lightingAvgBfsQueueSize.toFixed(1)} max ${stats.lightingMaxBfsQueueSize}`,
      '',
      '-- Block updates --',
