...
export interface WorldMetadata {
  ...
  /**
   * Dimension the player is currently in. Absent in saves written before
   * dimension support existed, which must load as the Overworld with no
   * migration pass.
   */
  readonly playerDimension?: number;
...
export function validateWorldMetadata(value: unknown): WorldMetadata {
  ...
  data.gameMode = parseGameMode(data.gameMode);
  // Saves written before dimension support have no playerDimension; they must
  // open as the Overworld without any migration pass.
  if (!Number.isInteger(data.playerDimension)) data.playerDimension = 0;
  ...
  if (typeof data.cheatsEnabled !== 'boolean') data.cheatsEnabled = false;
