...
export function classifyBlockPassMask(blockId: BlockId, registry: BlockRegistry): number {
  ...
  if (renderType === 'portal') return ChunkPassMask.Portal;
  if (
    renderType === 'cutout'
    || renderType === 'cross'
    || renderType === 'cactus'
...
export function computeChunkPassMask(blocks: Uint8Array, registry: BlockRegistry): number {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (mask === (ChunkPassMask.Terrain | ChunkPassMask.Water | ChunkPassMask.Lava | ChunkPassMask.Cutout | ChunkPassMask.Fire | ChunkPassMask.Translucent | ChunkPassMask.Leaves | ChunkPassMask.Portal)) {
      break;
    }
  }
  return mask;
